...
function usePokemonSource(): {
  ...
  type PokemonState = { pokemon: Pokemon[]; search: string; };
  ...
  useEffect(() => {
    fetch("/pokemon.json").then((response) => response.json()).then((data) =>
      dispatch({
        type: "setPokemon",
        payload: data,
      })
  ...
  const filteredPokemon = useMemo(() => 
    pokemon.filter((p) => p.name.toLowerCase().includes(search.toLowerCase())).slice(0, 20),
    [pokemon, search]
  );
